...
class FaceSwapNode(BaseVisualizerNode):
    def __init__(
        self,
        name: str,
        input_buffer: str,
    ...
    def draw(self, frame_msg):
        ...
        if num_target > 1:
            ...
            for tar_idx, src_idx in enumerate(self.history['target_map']):
                face_src = self._get_face_info(objects[src_idx])
                face_tar = self._get_face_info(objects[tar_idx])
    ...
    def _swap_face(self, img_src, img_tar, face_src, face_tar):
        ...
        patch_warp = cv2.warpAffine(patch_src,
                                    trans_mat,
                                    dsize=tuple(bbox_tar[2:] - bbox_tar[:2]),
                                    borderValue=(0, 0, 0))
        mask_warp = cv2.warpAffine(mask_src,
                                   trans_mat,
                                   dsize=tuple(bbox_tar[2:] - bbox_tar[:2]),
                                   borderValue=(0, 0, 0))
    ...
    @staticmethod
    def _get_face_info(obj):
        keypoints = obj['keypoints'][:, :2]
        model_cfg = obj['pose_model_cfg']
