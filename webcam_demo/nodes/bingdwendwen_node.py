...
from mmpose.apis.webcam.utils import load_image_from_disk_or_url
...
class BingDwenDwenNode(BaseVisualizerNode):
    ...
    def __init__(
        ...
        super().__init__(name, input_buffer, output_buffer, enable=True)
        ...
        self.register_event(self.mode_key,
                            is_keyboard=True,
                            handler_func=self.switch_mode)
    ...
    def _init_resource(self, resource_file):
        ...
        for ann in anns:
            ...
            keypoints = sorted(rst['step_1']['result'],
            ...
            mask = cv2.fillPoly(np.zeros(img.shape[:2], dtype=np.uint8),
    ...
    def draw(self, frame_msg):
        ...
        objects = frame_msg.get_objects(lambda x: 'keypoints' in x)
        ...
        if obj is not None:
            dataset = DatasetInfo(
                obj['pose_model_cfg'].data.test.dataset_info).dataset_name
            if dataset == 'ap10k':
                # left eye: 0, right eye: 1, nose: 2
                kpts_src = obj['keypoints'][[0, 1, 2], :2]
            elif dataset == 'coco_wholebody':
                # left eye: 1, right eye 2, nose: 0
                kpts_src = obj['keypoints'][[1, 2, 0], :2]
            else:
    ...
    def _get_transform(self, kpts_src, kpts_tar):
        if self.rigid:
            ...
            trans_mat, _ = cv2.estimateAffine2D(kpts_src.astype(np.float32),
                                                kpts_tar.astype(np.float32))
